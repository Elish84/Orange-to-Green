...
// ✅ חיסכון קריאות במפה: לא למשוך את כל ה-pins
const MAP_PINS_LIMIT = 1200; // תגדיל/תקטין לפי צורך
// ✅ גם לאדמין לא חייבים תמיד “הכל” (תוכל להחזיר לבלי limit אם ממש צריך)
const ADMIN_ROWS_LIMIT = 2500;
...
document.querySelectorAll(".tab").forEach((btn) => {
  btn.addEventListener("click", async () => {
    ...
    // ✅ ניהול listener של המפה רק כשהטאב פעיל (חיסכון קריאות!)
    if (currentTab !== "map" && tab === "map") {
      initMapIfNeeded();
      startPublicMapListenerIfNeeded();
    }
    if (currentTab === "map" && tab !== "map") {
      stopPublicMapListener();
    }
...
function stopPublicMapListener() {
  try {
    unsubMap?.();
  } catch (_) {}
  unsubMap = null;
...
/**
 * ✅ Backfill חסכוני: רץ פעם אחת בסשן + מוגבל כמות כדי לא לשרוף Writes.
 * אם יש לך הרבה היסטוריה – עדיף לעשות “מיגרציה חד פעמית” ולא בזמן אמת.
 */
let _backfillDoneThisSession = false;
let _backfillRunning = false;
async function backfillMapPinsFromHouseScansOnce(rows) {
  if (!isAdmin) return;
  if (_backfillDoneThisSession) return;
  if (_backfillRunning) return;
  if (!Array.isArray(rows) || rows.length === 0) return;

  _backfillRunning = true;
  try {
    // ✅ רק X הראשונים (הכי עדכניים) כדי לא להציף
    const subset = rows.slice(0, 400);
    let count = 0;
    ...
    console.log("backfillMapPinsFromHouseScansOnce done:", count);
    _backfillDoneThisSession = true;
  } finally {
    _backfillRunning = false;
  }
}

function startPublicMapListenerIfNeeded() {
  // ✅ רק אם טאב מפה פעיל
  if (currentTab !== "map") return;

  // ✅ guard נגד מאזינים כפולים
  if (unsubMap) return;

  // ✅ מושכים רק את האחרונים + limit (חיסכון קריאות)
  const q = query(
  ...
  unsubMap = onSnapshot(
    ...
    (snap) => {
      ...
      // צייר רק אם אנחנו עדיין במפה
      if (currentTab === "map") renderMap(mapRows);
    },
    (err) => {
      console.error(err);
      const hint = $("mapHint");
...
function startAdminListener() {
  ...
  // ✅ limit לאדמין כדי למנוע "הכל" בלי סוף (אפשר להגדיל)
  const q = query(
    collection(db, COLLECTION_NAME),
    orderBy("eventTimeISO", "desc"),
    limit(ADMIN_ROWS_LIMIT)
  );
  ...
  unsubAdmin = onSnapshot(
    ...
    (snap) => {
      ...
      // ✅ backfill חסכוני (רק פעם אחת בסשן)
      backfillMapPinsFromHouseScansOnce(rows);

      renderDashboard(rows);
      renderRecords(rows);

      // אדמין מצייר כמו כולם: לפי mapPins (אם המפה פתוחה)
      if (currentTab === "map") renderMap(mapRows);
    },
    (err) => {
      console.error(err);
...
onAuthStateChanged(auth, async (u) => {
  ...
  // - מפה: רק כשנכנסים לטאב מפה (חיסכון קריאות)
  ...
  // אם המשתמש כבר נמצא על טאב מפה בזמן שינוי auth — נרים listener
  startPublicMapListenerIfNeeded();
});
