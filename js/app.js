...
function setRtStatus(ok, text) {
  ...
  rtEl.classList.toggle("offline", !navigator.onLine);
  
  if (!navigator.onLine) {
    rtEl.textContent = "מצב לא מקוון";
...
window.addEventListener("online", () => {
  setRtStatus(true, "מחובר ל-DB");
  syncPendingForms();
});

window.addEventListener("offline", () => {
  setRtStatus(false, "מצב לא מקוון");
});
...
$("scanForm")?.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (!navigator.onLine) {
      await OfflineManager.enqueuePendingForm("houseScan", payload);
      if (toast) setToast(toast, "ok", "אין חיבור. הנתונים נשמרו מקומית ויסונכרנו בהמשך 💾");
      $("btnReset")?.click();
      updatePendingUI();
      return;
    }
    ...
    console.error(err);
    // Even if we thought we were online, if it fails due to network, save locally
    try {
      await OfflineManager.enqueuePendingForm("houseScan", payload);
      if (toast) setToast(toast, "ok", "שגיאה בחיבור. הנתונים נשמרו מקומית 💾");
      $("btnReset")?.click();
      updatePendingUI();
    } catch (e2) {
      console.error("Local save failed too:", e2);
      if (toast) setToast(toast, "bad", "שגיאה קריטית בשמירה (בדוק הרשאות/אחסון)");
    }
  } finally {
...
async function syncPendingForms() {
  ...
  const pending = await OfflineManager.getPendingForms();
  ...
  for (const item of pending) {
    try {
      ...
      // Use localId as doc ID if possible to prevent duplicates on retries,
      // but Firestore addDoc generates random IDs. 
      // To strictly prevent duplicates, we use localId for setDoc.
      const docRef = doc(db, COLLECTION_NAME, item.localId);
      
      // Replace serverTimestamp() with valid values because serverTimestamp() 
      // only works when online. If they were generated offline, we might need 
      // to adjust them or just use the payload's timestamp if it has one.
      // Actually payload has createdAt = serverTimestamp() from the form logic.
      // We'll update it to real date if it was saved locally.
      if (payload.createdAt && typeof payload.createdAt === 'object') {
        payload.createdAt = serverTimestamp();
      }
      payload.updatedAt = serverTimestamp();
      payload.isSyncedOffline = true;

      await setDoc(docRef, payload);
      await syncMapPinForRow(item.localId, payload);
      
      await OfflineManager.markFormSynced(item.localId);
      console.log(`Synced: ${item.localId}`);
    } catch (err) {
      console.error(`Sync failed for ${item.localId}:`, err);
      await OfflineManager.markSyncFailed(item.localId, err.message);
    }
  }
  
  _isSyncing = false;
  updatePendingUI();
  // We don't alert "Sync Complete" unless we want to, but standard behavior 
  // is just to update the UI indicators.
}

// Initial Sync & UI Update
updatePendingUI();
syncPendingForms();
setInterval(syncPendingForms, 60000); // Retry every minute if online
        ...
        options: {
          ...
          plugins: { 
            ...
            datalabels: {
              color: "#fff",
              font: { weight: "bold", size: 13 },
              formatter: (v) => v > 0 ? v : "" // Hide 0 values in pie
            }
          }
        }
    });
...
function renderDashboard(rows) {
  ...
  if (chartBySector && chartDetention) {
    // Generate high-contrast colors dynamically using HSL and Golden Angle
    ...
    chartDetention.data.datasets[0].backgroundColor = chartColors;
    chartDetention.data.datasets[0].borderColor = "rgba(15,24,49,0.9)";
    chartDetention.data.datasets[0].borderWidth = 2;
...
function makeAdminOptionRow(name, type) {
  ...
  // Only "אחר" is now a protected system sector
  const isDefault = type === "sector" && name === "אחר";
  
  return `
    <div class="optionItem">
      <div class="meta">
        <div class="name">${safeName}</div>
        <div class="sub">${label} ${isDefault ? 'קבוע (מערכת)' : 'פעיל לבחירה'}</div>
      </div>
      ${!isDefault ? `<button class="iconBtn danger" data-act="del-opt" data-type="${type}" data-name="${name}" title="מחיקה">✕</button>` : ''}
    </div>`;
}

function isOptionInUse(name, type) {
  const key = getOptionKey(name);
  if (type === "place") {
    return liveRows.some(r => getPlaceKey(r.place) === key);
  } else {
    return liveRows.some(r => getSectorKey(r.sector) === key);
  }
}

async function deleteAdminOption(name, type) {
  if (!(await ensureAdminOrLogin())) return;

  const label = type === "place" ? "המקום" : "הגזרה";
  const col = type === "place" ? PLACES_COLLECTION : SECTORS_COLLECTION;
  const docId = ensureOptionDocId(name, type);

  // 1) Usage Check
  if (isOptionInUse(name, type)) {
    alert(`לא ניתן למחוק ${label} "${name}" כיוון שיש רשומות המשתמשות בו.\nיש לשנות את הרשומות הקיימות לפני המחיקה.`);
    return;
  }

  // 2) Confirmation
  if (!confirm(`האם אתה בטוח שברצונך למחוק את ${label} "${name}"?`)) return;

  try {
    await deleteDoc(doc(db, col, docId));
    setToast($(type === "place" ? "adminPlaceToast" : "adminSectorToast"), "ok", `${label} נמחק בהצלחה`);
    // Snapshot listeners will trigger UI refresh
  } catch (e) {
    console.error(e);
    alert("שגיאה במחיקה מה-DB");
  }
}
...
function renderAdminPanel() {
  ...
  const pList = $("adminPlacesList");
  const sList = $("adminSectorsList");
  
  if (pList) {
    pList.innerHTML = places.length ? places.map((n) => makeAdminOptionRow(n, "place")).join("") : '<div class="optionItem emptyItem">עדיין אין מקומות</div>';
  }
  if (sList) {
    sList.innerHTML = sectors.length ? sectors.map((n) => makeAdminOptionRow(n, "sector")).join("") : '<div class="optionItem emptyItem">עדיין אין גזרות</div>';
  }

  // Add listeners
  document.querySelectorAll('button[data-act="del-opt"]').forEach(btn => {
    btn.addEventListener("click", () => {
      deleteAdminOption(btn.dataset.name, btn.dataset.type);
    });
  });
}
