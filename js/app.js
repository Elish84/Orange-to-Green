...
function setAddModeUI(active) {
  const enterBtn = $("btnEnterAddMode") || $("addTargetsBtn");
  const exitBtn = $("btnExitAddMode") || $("exitAddTargetsBtn");
...
function initAddTargetsModeUI() {
  ...
  enterBtn?.addEventListener("click", () => enterAddTargetsMode());
  exitBtn?.addEventListener("click", () => exitAddTargetsMode());
...
// avoid duplicate realtime subscriptions (e.g., after admin login)
let rtUnsub = null;

function initCharts() {
  const c1 = $("chartBySector");
  const c2 = $("chartDetention");
  if (!c1 || !c2) return;
  ...
  try {
    if (chartDetention) { chartDetention.destroy(); chartDetention = null; }
  } catch (_) { chartDetention = null; }

  chartBySector = new Chart(c1, {
    type: "bar",
    data: {
      labels: SECTORS,
...
function startRealtimeListener() {
  // prevent duplicated listeners after auth state changes
  if (rtUnsub) {
    try { rtUnsub(); } catch (_) {}
    rtUnsub = null;
  }
